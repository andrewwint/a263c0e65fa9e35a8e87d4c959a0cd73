...
Project-local precedent: `predict_user_rating` originally pulled the user's 10 most recent ratings *including the target movie*. For any pair where the user had already rated the movie, the LLM saw the ground-truth answer in its prompt and echoed it. Fix: `AND rt.movieId != ?` in the history query. This is exactly the kind of bug the "write a test for the leak-adjacent case" rule exists to catch — it's a classic data-leakage pattern that should have been caught at code review, not burned through an eval run.
