...
- **Plan-then-build.** PLAN.md / TASKS.md / AGENTS.md committed as the initial commit (`31da6d4`), before any code. Gave reviews (mine, Amazon Q's, Google Gemini's, user's) a shared document to push on, and made mid-project revisions explicit (model swap, Haiku version, schema docs) — the plan was a contract, not prose.
